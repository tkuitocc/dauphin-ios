            ...
            List(filteredLocations, id: \.id) { loc in
                Button {
                    ...
                    HStack {
                        ...
                        Text(verbatim: "\(loc.code == "ZZZ" ? "" : loc.code)").font(.headline)
                            .frame(
                                width: UIFont.preferredFont(forTextStyle: .headline).pointSize * 2,
                                alignment: .leading
                            )
                        Text(verbatim: "\(loc.name)").font(.headline)
                        Spacer()
                        Image(systemName: "chevron.right").font(.footnote).foregroundStyle(
                            .tertiary)
                    }.padding(2)
                }.accessibilityLabel(Text(verbatim: "\(loc.code) \(loc.name)")).accessibilityHint(
