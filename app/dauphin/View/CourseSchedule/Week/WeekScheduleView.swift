...
struct WeekScheduleView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            VStack {
                ...
                HStack(spacing: 0) {
                    Spacer().frame(width: 45)  // Match time label width

                    WeekdaysView(
                        days: dayLabels, weekdays: displayedWeekdays, width: dayWidth,
                    ...
                    HStack(spacing: 0) {
                        ...
                        ForEach(Array(filteredCourses.enumerated()), id: \.offset) { _, courses in
                            TimelineView(
                                ...
                                onCourseTap: { course in handleCourseTap(course) })
                        }
                    }
                }
            }
    ...
    private func handleCourseTap(_ course: Course) { selectedCourse = course }
}
