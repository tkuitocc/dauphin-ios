...
@Suite("Next Up Service Tests / 下堂課排序與過濾")
struct CourseNextUpServiceTests {
  ...
  func orderSameDayByStartTime() {
    ...
    let result = dauphin.DefaultNextUpService().nextUp(
      from: [c2, c3, c1], now: reference)
    #expect(result.map { $0.name } == ["C3", "C2", "C1"])
  }

  @Test("星期日中午後若有課則保留本週日課程")
  func sundayNoonKeepsSundayCourse() {
