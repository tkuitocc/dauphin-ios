...
struct TimelineView: View {
    ...
    private var positionedCourses: [CoursePosition] {
        ...
        for course in sortedCourses {
            ...
            for i in 0 ..< groups.count {
                ...
                for existingCourse in group where coursesOverlap(course, existingCourse) {
                    hasOverlap = true
                    break
                }
        ...
        for group in groups {
            ...
            for course in group {
                ...
                for i in 0 ..< overlapSets.count {
                    ...
                    for setCourse in overlapSets[i] where coursesOverlap(course, setCourse) {
                ...
                if !addedToSet { overlapSets.append([course]) }
            }
            ...
            for overlapSet in overlapSets {
                ...
                for (index, course) in overlapSet.enumerated() {
                    positions.append(
                        CoursePosition(course: course, column: index, totalColumns: totalColumns))
                }
            }
        }
                ...
                ForEach(positionedCourses, id: \.course.id) { position in
                    ...
                    GeometryReader { geo in
                        ...
                        if position.totalColumns == 1 {
                            ...
                            CourseView(course: position.course, height: cardHeight, yOffset: yBase)
                                .frame(width: courseWidth).offset(x: xOffset).onTapGesture {
                            ...
                            let courseWidth: CGFloat =
                                ...
                                ? availableWidth / CGFloat(position.totalColumns) : totalWidth
    ...
    private func normalized(_ date: Date) -> Date {
        ...
        return calendar.date(
            ...
            second: components.second ?? 0, of: calendar.startOfDay(for: start)) ?? date
    }
...
#Preview {
    ...
    ScrollView {
        TimelineView(
            courses: Binding(
                get: { courseViewModel.weekCourses.filter { $0.weekday == 1 } }, set: { _ in }))
    }
}
